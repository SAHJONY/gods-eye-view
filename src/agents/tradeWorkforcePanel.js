 ...
 * initTradeWorkforcePanel(). This module intentionally does NOT touch
 * Cesium — the shipping layer owns the 3D entities.
 */
...
export const AGENT_DEFS = [
  {
    ...
    desc: {
      es: 'Triagea nuevos RFQs y valida los datos de entrada.',
      en: 'Triages new RFQs and validates intake data.',
    },
  },
  {
    id: 'rfq-researcher',
    ...
    desc: {
      es: 'Listas de diligencia de contrapartes (notas, nadie contactado).',
      en: 'Counterparty diligence checklists (notes only, nobody contacted).',
    },
  },
  {
    ...
    desc: {
      es: 'Costo landed, margen y memos del trato.',
      en: 'Landed cost, margin, and deal memos.',
    },
  },
  {
    id: 'deal-coordinator',
 ...
 * Accepts the real trade workforce ({ agents: [...], running getter,
...
const TWF_CSS = `
#gev-trade-workforce-btn{display:flex;align-items:center;gap:8px;padding:10px 12px;margin-top:8px;background:rgba(10,14,22,.82);border:1px solid rgba(56,189,248,.32);border-radius:14px;backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);color:#f2f4f8;font-family:inherit;cursor:pointer;font-size:12px;font-weight:700;letter-spacing:.06em;min-width:172px;max-width:210px}
#gev-trade-workforce-btn:hover{border-color:rgba(56,189,248,.65)}
#gev-trade-workforce-panel{position:fixed;z-index:60;left:12px;top:12px;width:min(400px,calc(100vw - 24px));height:min(600px,calc(100vh - 24px));background:rgba(8,11,18,.96);border:1px solid rgba(56,189,248,.32);border-radius:16px;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 12px 40px rgba(0,0,0,.6);font-family:inherit;color:#eef2f7}
#gev-trade-workforce-panel .cwp-head{display:flex;align-items:center;gap:8px;padding:10px 12px;border-bottom:1px solid rgba(56,189,248,.2)}
#gev-trade-workforce-panel .cwp-title{font-size:12px;font-weight:800;letter-spacing:.1em;color:#7dd3fc;flex:1}
#gev-trade-workforce-panel .cwp-lang{background:none;border:1px solid rgba(56,189,248,.4);color:#7dd3fc;border-radius:8px;padding:5px 10px;cursor:pointer;font-size:11px;font-weight:800;font-family:inherit}
#gev-trade-workforce-panel .cwp-close{background:none;border:1px solid rgba(255,255,255,.2);color:#fff;border-radius:8px;width:28px;height:28px;cursor:pointer;font-size:14px}
#gev-trade-workforce-panel .cwp-controls{display:flex;align-items:center;gap:8px;padding:10px 12px;border-bottom:1px solid rgba(56,189,248,.2)}
#gev-trade-workforce-panel .cwp-btn{padding:7px 14px;border-radius:9px;border:1px solid rgba(56,189,248,.4);background:rgba(56,189,248,.1);color:#e0f2fe;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit}
#gev-trade-workforce-panel .cwp-btn:hover{background:rgba(56,189,248,.22)}
#gev-trade-workforce-panel .cwp-queue{font-size:11px;color:#9fb0c9;margin-left:auto}
#gev-trade-workforce-panel .cwp-roster{padding:6px 12px;border-bottom:1px solid rgba(56,189,248,.2);max-height:280px;overflow-y:auto}
#gev-trade-workforce-panel .cwp-agent{display:flex;gap:10px;padding:9px 4px;border-bottom:1px solid rgba(56,189,248,.08)}
#gev-trade-workforce-panel .cwp-agent:last-child{border-bottom:none}
#gev-trade-workforce-panel .dot{width:10px;height:10px;border-radius:50%;margin-top:4px;flex:none}
#gev-trade-workforce-panel .dot.idle{background:#64748b}
#gev-trade-workforce-panel .dot.working{background:#34d399;box-shadow:0 0 8px rgba(52,211,153,.8)}
#gev-trade-workforce-panel .dot.paused{background:#fbbf24}
#gev-trade-workforce-panel .cwp-agent .aname{font-size:12px;font-weight:800;color:#fff}
#gev-trade-workforce-panel .cwp-agent .astatus{font-size:10px;font-weight:700;letter-spacing:.06em;color:#7dd3fc;text-transform:uppercase}
#gev-trade-workforce-panel .cwp-agent .adesc{font-size:11px;color:#9fb0c9;margin-top:2px}
#gev-trade-workforce-panel .cwp-agent .alast{font-size:10px;color:#5b6b82;margin-top:2px;font-style:italic}
#gev-trade-workforce-panel .cwp-feedhead{font-size:10px;font-weight:800;letter-spacing:.08em;color:#7dd3fc;padding:10px 12px 4px}
#gev-trade-workforce-panel .cwp-feed{flex:1;overflow-y:auto;padding:4px 12px 10px;display:flex;flex-direction:column;gap:6px;min-height:0}
#gev-trade-workforce-panel .cwp-item{background:rgba(56,189,248,.05);border:1px solid rgba(56,189,248,.14);border-radius:9px;padding:7px 10px;font-size:11px;color:#c8d4e4}
#gev-trade-workforce-panel .cwp-item .fts{font-size:10px;color:#5b6b82}
#gev-trade-workforce-panel .cwp-note{padding:10px 12px;border-top:1px solid rgba(56,189,248,.2);font-size:10px;color:#8fa3bd;line-height:1.5;background:rgba(56,189,248,.04)}
`;

function injectStyles() {
  if (typeof document === 'undefined') return;
  if (document.getElementById('gev-trade-workforce-styles')) return;
  const style = document.createElement('style');
  style.id = 'gev-trade-workforce-styles';
  style.textContent = TWF_CSS;
...
/**
 * Live controller: dock button + floating trade AI workforce panel.
 * options: { workforce, signal }
 *
 * workforce shape (all optional, panel stays honest without one).
 * Accepts the real trade workforce ({ agents: [...], running,
 * onActivity(), getActivityLog(), getSummary(), start/pause/resume }) as
 * well as the minimal double shape ({ agents()/feed()/queueDepth()/
 * running()/start/pause/resume }). See adaptWorkforce() for the
 * normalization.
 */
export function initTradeWorkforcePanel({
  workforce = null,
  signal = null,
} = {}) {
  injectStyles();
  ...
  function open() {
    ...
    panelEl.id = 'gev-trade-workforce-panel';
    panelEl.innerHTML = `
      <div class="cwp-head">
        <div class="cwp-title">COMERCIO · AGENTES IA</div>
  ...
  if (typeof document !== 'undefined') {
    ...
    btnEl.id = 'gev-trade-workforce-btn';
    btnEl.type = 'button';
    btnEl.innerHTML =
      '<span style="font-size:20px">🤖</span><span>Comercio</span>';
    btnEl.setAttribute('aria-label', 'Comercio agentes IA / Trade AI agents');
  ...
  if (typeof window !== 'undefined') window.__gevTradeWorkforceUI = api;
  return api;
}
