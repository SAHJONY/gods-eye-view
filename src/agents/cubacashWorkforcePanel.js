 ...
 * initCubacashWorkforcePanel(). This module intentionally does NOT touch
...
export const AGENT_DEFS = [
  {
    ...
    desc: {
      es: 'Revisa datos de candidatos y deja listas de verificación manual.',
      en: 'Reviews candidate data and leaves manual verification checklists.',
    },
  },
  {
    id: 'fee-watcher',
    ...
    desc: {
      es: 'Marca proveedores en vivo con tarifas sin declarar — nunca inventa precios.',
      en: 'Flags live providers with undisclosed fees — never invents prices.',
    },
  },
  {
    id: 'corridor-analyst',
    ...
    desc: {
      es: 'Memos de corredor: comisión, neto y recibido por tasa.',
      en: 'Corridor memos: fee, net, and received amount per rate.',
    },
  },
  {
    id: 'intake-helper',
    ...
    desc: {
      es: 'Borradores internos de contacto — nunca se envían solos.',
      en: 'Internal outreach drafts — never sent on their own.',
    },
  },
];
 ...
 * Accepts the real cubacash workforce ({ agents: [...], running getter,
...
export function adaptWorkforce(raw) {
  ...
  return {
    agents() {
      ...
      // No workforce connected: show the 4 canonical cubacash roles, all
      // idle, so the roster is never a lie about activity.
      if (!out.length) {
...
const CWF_CSS = `
#gev-cubacash-workforce-btn{display:flex;align-items:center;gap:8px;padding:10px 12px;margin-top:8px;background:rgba(10,14,22,.82);border:1px solid rgba(255,180,84,.32);border-radius:14px;backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);color:#f2f4f8;font-family:inherit;cursor:pointer;font-size:12px;font-weight:700;letter-spacing:.06em;min-width:172px;max-width:210px}
#gev-cubacash-workforce-btn:hover{border-color:rgba(255,180,84,.65)}
#gev-cubacash-workforce-panel{position:fixed;z-index:60;left:12px;top:12px;width:min(400px,calc(100vw - 24px));height:min(600px,calc(100vh - 24px));background:rgba(8,11,18,.96);border:1px solid rgba(255,180,84,.32);border-radius:16px;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 12px 40px rgba(0,0,0,.6);font-family:inherit;color:#eef2f7}
#gev-cubacash-workforce-panel .cwp-head{display:flex;align-items:center;gap:8px;padding:10px 12px;border-bottom:1px solid rgba(255,180,84,.2)}
#gev-cubacash-workforce-panel .cwp-title{font-size:12px;font-weight:800;letter-spacing:.1em;color:#ffb454;flex:1}
#gev-cubacash-workforce-panel .cwp-lang{background:none;border:1px solid rgba(255,180,84,.4);color:#ffb454;border-radius:8px;padding:5px 10px;cursor:pointer;font-size:11px;font-weight:800;font-family:inherit}
#gev-cubacash-workforce-panel .cwp-close{background:none;border:1px solid rgba(255,255,255,.2);color:#fff;border-radius:8px;width:28px;height:28px;cursor:pointer;font-size:14px}
#gev-cubacash-workforce-panel .cwp-controls{display:flex;align-items:center;gap:8px;padding:10px 12px;border-bottom:1px solid rgba(255,180,84,.2)}
#gev-cubacash-workforce-panel .cwp-btn{padding:7px 14px;border-radius:9px;border:1px solid rgba(255,180,84,.4);background:rgba(255,180,84,.1);color:#ffe9c9;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit}
#gev-cubacash-workforce-panel .cwp-btn:hover{background:rgba(255,180,84,.22)}
#gev-cubacash-workforce-panel .cwp-queue{font-size:11px;color:#9fb0c9;margin-left:auto}
#gev-cubacash-workforce-panel .cwp-roster{padding:6px 12px;border-bottom:1px solid rgba(255,180,84,.2);max-height:280px;overflow-y:auto}
#gev-cubacash-workforce-panel .cwp-agent{display:flex;gap:10px;padding:9px 4px;border-bottom:1px solid rgba(255,180,84,.08)}
#gev-cubacash-workforce-panel .cwp-agent:last-child{border-bottom:none}
#gev-cubacash-workforce-panel .dot{width:10px;height:10px;border-radius:50%;margin-top:4px;flex:none}
#gev-cubacash-workforce-panel .dot.idle{background:#64748b}
#gev-cubacash-workforce-panel .dot.working{background:#34d399;box-shadow:0 0 8px rgba(52,211,153,.8)}
#gev-cubacash-workforce-panel .dot.paused{background:#fbbf24}
#gev-cubacash-workforce-panel .cwp-agent .aname{font-size:12px;font-weight:800;color:#fff}
#gev-cubacash-workforce-panel .cwp-agent .astatus{font-size:10px;font-weight:700;letter-spacing:.06em;color:#ffb454;text-transform:uppercase}
#gev-cubacash-workforce-panel .cwp-agent .adesc{font-size:11px;color:#9fb0c9;margin-top:2px}
#gev-cubacash-workforce-panel .cwp-agent .alast{font-size:10px;color:#5b6b82;margin-top:2px;font-style:italic}
#gev-cubacash-workforce-panel .cwp-feedhead{font-size:10px;font-weight:800;letter-spacing:.08em;color:#ffb454;padding:10px 12px 4px}
#gev-cubacash-workforce-panel .cwp-feed{flex:1;overflow-y:auto;padding:4px 12px 10px;display:flex;flex-direction:column;gap:6px;min-height:0}
#gev-cubacash-workforce-panel .cwp-item{background:rgba(255,180,84,.05);border:1px solid rgba(255,180,84,.14);border-radius:9px;padding:7px 10px;font-size:11px;color:#c8d4e4}
#gev-cubacash-workforce-panel .cwp-item .fts{font-size:10px;color:#5b6b82}
#gev-cubacash-workforce-panel .cwp-note{padding:10px 12px;border-top:1px solid rgba(255,180,84,.2);font-size:10px;color:#8fa3bd;line-height:1.5;background:rgba(255,209,102,.04)}
`;

function injectStyles() {
  if (typeof document === 'undefined') return;
  if (document.getElementById('gev-cubacash-workforce-styles')) return;
  const style = document.createElement('style');
  style.id = 'gev-cubacash-workforce-styles';
...
/**
 * Live controller: dock button + floating MY CUBA CASH AI workforce panel.
 * options: { workforce, signal }
 *
 * workforce shape (all optional, panel stays honest without one).
 * Accepts the real cubacash workforce ({ agents: [...], running,
 * onActivity(), getActivityLog(), getSummary(), start/pause/resume }) as
 * well as the minimal double shape ({ agents()/feed()/queueDepth()/
 * running()/start/pause/resume }). See adaptWorkforce() for the
 * normalization.
 */
export function initCubacashWorkforcePanel({
  workforce = null,
  signal = null,
} = {}) {
  injectStyles();
  ...
  function open() {
    ...
    panelEl.id = 'gev-cubacash-workforce-panel';
    panelEl.innerHTML = `
      <div class="cwp-head">
        <div class="cwp-title">CUBACASH · AGENTES IA</div>
  ...
  if (typeof document !== 'undefined') {
    ...
    btnEl.id = 'gev-cubacash-workforce-btn';
    btnEl.type = 'button';
    btnEl.innerHTML =
      '<span style="font-size:20px">🤖</span><span>Agentes IA</span>';
    btnEl.setAttribute(
      'aria-label',
      'MY CUBA CASH agentes IA / MY CUBA CASH AI agents',
    );
  ...
  if (typeof window !== 'undefined') window.__gevCubacashWorkforceUI = api;
  return api;
}
